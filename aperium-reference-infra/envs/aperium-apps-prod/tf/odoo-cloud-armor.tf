locals {
  ...
  odoo_cloud_armor_allowlist_chunks = [
...
resource "google_compute_security_policy" "odoo_allowlist" {
  count = var.enable_odoo_cloud_armor_policy ? 1 : 0

  name        = var.odoo_cloud_armor_policy_name
  description = "Cloud Armor allowlist policy for Odoo ERP Gateway access."

  dynamic "rule" {
    for_each = local.odoo_cloud_armor_allowlist_chunks
